...
{
    builder.Services.AddSingleton<ITelegramReportSender>(_ => new TelegramReportSender(
        token!,
        chatId!
    ));
}
...
builder.Services.AddScoped<
    Memes.Infrastructure.Reporting.IDailyReportJob,
...
builder.Services.AddSingleton(sp =>
{
    ...
    var chatId =
        cfg["Telegram:ChatId"] ?? throw new InvalidOperationException("Telegram:ChatId missing");
    ...
    while (true)
    {
        ...
        catch (Exception ex) when (attempts < maxAttempts)
        {
            ...
            app.Logger.LogWarning(
                ex,
                "DB not ready (attempt {Attempt}/{Max}). Retrying in {Delay}s...",
                attempts,
                maxAttempts,
                delay.TotalSeconds
            );
...
app.MapGet(
        "/memes/top-24h",
        async (int? take, GetTopMemesLast24h uc, CancellationToken ct) =>
        {
...
app.MapGet(
        "/reports/top-24h",
        async (int? take, IMemeRepository repo, CancellationToken ct) =>
        {
            var list = await repo.GetTopFromDbLast24hAsync(
                take is >= 1 and <= 100 ? take.Value : 20,
                ct
            );
...
app.MapPost(
    "/debug/send-text",
    async (ITelegramReportSender sender, CancellationToken ct) =>
    {
